...
WebUnzipper 是一套部署在 Linux 服务器上的高性能、全自动文件处理流与在线阅读中枢。

通过整合 Python 守护进程、P7zip 引擎、Samba 局域网共享与深度魔改的前端 Web 面板，本项目解决了从压缩包中自动解压的痛点（如密码提取、GBK 乱码、等）。
...
git clone https://github.com/BlueIris/web-unzipper.git
cd web-unzipper
赋予脚本执行权限并运行：
...
chmod +x install.sh
sudo ./install.sh
按照终端提示设置 Samba 密码与 FileBrowser 端口。

在浏览器中访问 http://<你的服务器IP>:8080，使用 reader 账号登录并“添加到主屏幕”以享受沉浸模式。
